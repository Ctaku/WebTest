...
if not(request.cookies(eChsys)("KEY")="super" or request.cookies(eChsys)("KEY")="typemaster" or request.cookies(eChsys)("KEY")="bigmaster" or request.cookies(eChsys)("key")="smallmaster" or request.cookies(eChsys)("key")="selfreg" or (request.cookies(eChsys)("KEY")="check" and checkmod="1"))  then
	...
	sql="select * from "& db_User_Table &" where username='"&request.cookies(eChsys)("username")&"'"
	...
	if rs("editor")<>request.cookies(eChsys)("username") and request.cookies(eChsys)("KEY")<>"super" And(request.cookies(eChsys)("KEY")<>"check" and checkmod="1" ) then
	...
	If (isnull(E_BigClassID) or isempty(E_BigClassID) or E_BigClassID="") Then
		E_BigClassID = 0
	End if
	...
	sql3="select * from "& db_EC_SmallClass_Table &" where E_SmallClassID="&E_SmallClassID
	...
	set rs1=nothing
	set rs2=server.CreateObject("ADODB.RecordSet")
	rs2.Source="select * From "& db_EC_BigClass_Table &" Where E_BigClassID=" &E_BigClassID
	rs2.Open rs2.Source,conn,1,1
	bigclasszs=rs2("bigclasszs")
	if rs("E_SmallClassID")<>"" then
	set rs3=server.CreateObject("ADODB.RecordSet")
	rs3.Source="select * From "& db_EC_SmallClass_Table &" Where E_SmallClassID=" &E_SmallClassID
	rs3.Open rs3.Source,conn,1,1
	end if
	if rs("checkked")="0" or request.cookies(eChsys)("key")="super" or (request.cookies(eChsys)("key")="bigmaster" and bigclassmaster5=true) or (request.cookies(eChsys)("key")="typemaster" and typemaster4=true) or (request.cookies(eChsys)("key")="check" and checkmod="1") or (modnewsable="1" and request.cookies(eChsys)("key")="smallmaster" and smallclassmaster6=true) or (request.cookies(eChsys)("key")="smallmaster" and rs("checkked")="1" and modnewsable="1" and smallclassmaster6=true) or (request.cookies(eChsys)("key")="selfreg" and jingyong=0 and fabiaomod="1") then
	%>
<html><head>
...
var range =message.document.selection.createRange();
str1="<font color='#FFFFFF'>[<%=rs8("name")%>]<%=RND()%><%=date()%></font>"
   range.pasteHTML(str1);
}
...
var range =message.document.selection.createRange();
str1="<TABLE cellSpacing=1 cellPadding=3 width=600 align=center bgColor=#AFAFD9 border=0><TBODY><TR><TD noWrap align=middle width=15% bgColor=#AFAFD9 height=20>软件名称：</TD><TD bgColor=#AFAFD9 colSpan=3 height=20>&nbsp; </TD></TR><TR bgColor=#fbfbfb><TD width='15%' height=20 align=middle noWrap>软件语言：</TD><TD width='35%' height=20>简体中文</TD><TD width='15%' align=middle vAlign=center>整理时间： </TD><TD width='35%' height=20>&nbsp;</TD></TR><TR bgColor=#fbfbfb><TD width='15%' height=20 align=middle noWrap>运行环境：</TD>      <TD width='35%' height=20>windowsXP/2003/2000/98/95x</TD><TD width='15%' align=middle vAlign=center>软件大小：</TD><TD width='35%' height=20>M</TD></TR><TR bgColor=#fbfbfb><TD width='15%' height=20 align=middle noWrap>软件等级：</TD>      <TD width='35%' height=20 bgcolor='#fbfbfb'><font color='#FF0000'>★★★</font></TD><TD width='15%' align=middle vAlign=center>授权方式：</TD><TD width='35%' height=20>免费</TD></TR><TR bgColor=#fbfbfb><TD width='15%' height=20 align=middle noWrap>开 发 商：</TD><TD colSpan=3 height=20>&nbsp;</TD></TR><TR bgColor=#fbfbfb><TD width='15%' height=20 align=middle vAlign=top noWrap>软件简介：</TD><TD colSpan=3 height=20>&nbsp;</TD></TR><TR bgColor=#ffffff><TD width='15%' height=20 align=middle vAlign=top noWrap bgColor=#AFAFD9>下载地址：</TD><TD bgColor=#AFAFD9 colSpan=3 height=20></TD></TR><TR bgColor=#ffffff><TD noWrap bgColor=#fbfbfb colSpan=4 height=20>&nbsp;</TD></TR></TBODY></TABLE>"
   range.pasteHTML(str1);
}
...
var range =message.document.selection.createRange();
str1="<TABLE cellSpacing=1 cellPadding=3 width=600 align=center bgColor=#AFAFD9 border=0><tr><td width='95'>产品名称：</td><td width='489'>&nbsp;</td></tr><tr bgcolor='#FFFFFF'><td>产品型号：</td><td>&nbsp;</td></tr><tr bgcolor='#FFFFFF'><td>产品规格：</td><td>&nbsp;</td></tr><tr bgcolor='#FFFFFF'><td>产品介绍：</td><td>&nbsp;</td></tr><tr bgcolor='#FFFFFF'><td>产品价格：</td><td>&nbsp;</td></tr></table>"
   range.pasteHTML(str1);
}
...
var range =message.document.selection.createRange();
str1="<TABLE cellSpacing=0 cellPadding=0 width=263 border=0 align=center><TBODY><TR><TD vAlign=top width=8><IMG height=7 src='images/index_t_center_1.gif' width=8></TD><TD width=248 bgColor=#c9d5ae></TD><TD vAlign=top width=7><IMG height=7 src='images/index_t_center_2.gif' width=7></TD></TR><TR><TD bgColor=#c9d5ae>&nbsp;</TD><TD align='center' valign='middle'>&nbsp; </TD><TD bgColor=#c9d5ae>&nbsp;</TD></TR><TR><TD><IMG height=8 src='images/index_t_center_4.gif' width=8></TD><TD width=248 bgColor=#c9d5ae></TD><TD><IMG height=8 src='images/index_t_center_3.gif' width=7></TD></TR></TBODY></TABLE>"
   range.pasteHTML(str1);
}
...
rs6.Source="select * from "& db_EC_Type_Table &" where E_typeid="&rs("E_typeid")
rs6.Open rs6.source,conn,1,1
E_typename=rs6("E_typename")
mode=rs6("mode")
rs6.close
set rs6=nothing
%>
<div align="center">
<center>
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="<%=border%>" bgcolor="#FFFFFF" style="border-collapse: collapse">
  <tr> 
    <td colspan="4" height="25" width="100%"  class="TDtop"> 
      <div align="center">┊ <B>在<b>[<a href="E_BigClassManage.asp?E_typeid=<%=rs("E_typeid")%>"><%=E_typename%></a>]<%if rs("E_BigClassID")<>"" then%>-[<a href="E_SmallClassManage.asp?E_BigClassID=<%=rs("E_BigClassID")%>"><%=rs2("E_BigClassName")%></a>]<%if rs("E_SmallClassID")<>"" then%>-[<a href="ListNews.asp?E_SmallClassID=<%=rs("E_SmallClassID")%>"><%=rs3("E_smallclassname")%></a>]<%end if%><%end if%></b>中修改文章，ID号为<%=newsid%></B> ┊</div>
    </td>
  </tr>
  <%if request.cookies(eChsys)("key")="super" or request.cookies(eChsys)("key")="typemaster" or request.cookies(eChsys)("key")="selfreg" then%>
  ...
  <form method="POST" OnSubmit="return checkdata()" onReset="return ResetForm();" name="form1" action="E_NewsEditOK.asp?NewsID=<%=NewsID%>">
...
rs4.Source="select * from "& db_EC_Special_Table &""
...
<%end if%>
    <tr> 
      ...
      <td bgcolor="#FFFFFF" colspan="3"> <span style='cursor:hand' title='缩短对话框' onClick='if (me.size>10)me.size=me.size-2'>-</span> 
        <input name="title" id=me type="TEXT" size=60 maxlength=100 style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("Title"))%>" onMouseOver="window.status='在这里修改文章标题';return true;" onMouseOut="window.status='';return true;" title="在这里修改文章标题">
...
<%if mode<>5 and bigclasszs<>5 then%>
    <tr> 
      ...
      <td bgcolor="#FFFFFF" colspan="3"> <span style='cursor:hand' title='缩短对话框' onClick='if (title_face.size>10)title_face.size=title_face.size-2'>-</span> 
        <input name="title_face" id=title_face type="TEXT" size=30 maxlength=100 style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("titleface"))%>" onMouseOver="window.status='文章为直接链接文章时，请填写网址';return true;" onMouseOut="window.status='';return true;" title="文章为直接链接文章时，请填写网址">
	...
	<tr> 
      ...
      <td bgcolor="#FFFFFF" colspan="3"> <span style='cursor:hand' title='缩短对话框' onClick='if (message.size>10)message.size=message.size-2'>-</span> 
        <input name="Original" id=message type="TEXT" size=30 maxlength=100 style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("Original"))%>" onMouseOver="window.status='按这个按钮修改文章来源';return true;" onMouseOut="window.status='';return true;" title="按这个按钮修改文章来源">
    ...
    <tr> 
      ...
      <td bgcolor="#FFFFFF" colspan="3"><span style='cursor:hand' title='缩短对话框' onClick='if (mess.size>10)mess.size=mess.size-2'>-</span> 
        <input name="Author" id=mess type="TEXT" size=30 maxlength=100 style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("Author"))%>" onMouseOver="window.status='按这个按钮修改文章作者';return true;" onMouseOut="window.status='';return true;" title="按这个按钮修改文章作者">
...
<option style="background-color:#9ACD32;color: #9ACD32" value="#9ACD32">#9ACD32</option>
 ...
 <select language="javascript"  id="FontSize" title="字号大小" onChange="FormatText('fontsize',this[this.selectedIndex].value);" name="select" onMouseOver="window.status='选择选定文字的字号大小。';return true;" onMouseOut="window.status='';return true;">
              ...
              </select><%if request.cookies(eChsys)("key")="super" then%><input onClick="setMode(this.checked);" name="viewhtml" type="checkbox" value="ON">查看HTML源代码<%end if%><br>
		&nbsp;
...
<!--增加引用样式结束-->
		...
		<img class=None onMouseDown="Check(this,2)" onMouseOver="Check(this,1)" onMouseOut="Check(this,0)" onMouseUp="Check(this,1)" src="images/software.gif" align="absmiddle" border="0" style="cursor:hand;" alt="插入软件下载版式" lANGUAGE="javascript" onClick="software()">
		<img class=None onMouseDown="Check(this,2)" onMouseOver="Check(this,1)" onMouseOut="Check(this,0)" onMouseUp="Check(this,1)" src="images/production.gif" align="absmiddle" border="0" style="cursor:hand;" alt="插入产品介绍版式" lANGUAGE="javascript" onClick="production()">
		<img class=None onMouseDown="Check(this,2)" onMouseOver="Check(this,1)" onMouseOut="Check(this,0)" onMouseUp="Check(this,1)" src="images/frame.gif" align="absmiddle" border="0" style="cursor:hand;" alt="插入图片像框版式" lANGUAGE="javascript" onClick="frame()">
		...
		<br>&nbsp;&nbsp;<iframe style="top:2px" ID="UploadFiles" src="upme.htm" frameborder=0 scrolling=no width="280" height="25"></iframe></div>
                           <script language="javascript">
                           var bTextMode=false;
		    	document.write ('<iframe src="textbox.asp?action=modify&newsid=<%=newsid%>" id="message" width="100%" height="315" align=center></iframe>')
...
<%if mode<>5 and bigclasszs<>5 then%>
    <tr> 
      ...
      <td bgcolor="#FFFFFF" colspan="3"><span style='cursor:hand' title='缩短对话框' onClick='if (ss.size>10)ss.size=ss.size-2'>-</span> 
        <input name="about" id=ss type="TEXT" size=30 maxlength=100 style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("about"))%>" onMouseOver="window.status='在这里修改关键字';return true;" onMouseOut="window.status='';return true;" title="在这里修改关键字">
	...
	<tr> 
		...
		<td width="680" bgcolor="#FFFFFF">&nbsp;
			<input name="PicUrl" type="text" id="PicUrl" size="30" maxlength="200" style=" height:22;background-color:ffffff;border: 1 double; border-color:#BDD0FD;" value="<%=trim(rs("picname"))%>">
		...
		sqls="select * from "& db_EC_Attach_Table &" where NewsID=" & NewsID 
    ...
    <%if request.cookies(eChsys)("key")="super" or request.cookies(eChsys)("key")="check" or request.cookies(eChsys)("key")="typemaster" or request.cookies(eChsys)("key")="bigmaster" then%>
    <tr> 
      ...
      <td valign="middle" bgcolor="#FFFFFF" colspan="3">&nbsp; 
        <input type="radio" value="1" <%if rs("daodu")=1 then Response.Write "checked"%>  name="daodu">
        是 
        <input type="radio" value="0" <%if rs("daodu")=0 then Response.Write "checked"%> name="daodu">
        否&nbsp;&nbsp;大标条头</td>
    </tr>
<tr> 
...
<tr> 
                ...
                <td valign="middle" bgcolor="#FFFFFF" >&nbsp; 
                  <input type="radio" value="1" <%if rs("backtype1")=1 then Response.Write "checked"%>  name="backtype1">
                  是 
                  <input type="radio" value="0" <%if rs("backtype1")=0 then Response.Write "checked"%> name="backtype1">
                  否&nbsp;&nbsp;内部浏览文章</td>
</tr>
...
<input name="checkked" type="hidden" value="<%if request.cookies(eChsys)("key")="super" or request.cookies(eChsys)("key")="check" or request.cookies(eChsys)("key")="typemaster" or request.cookies(eChsys)("key")="bigmaster" then%>1<%else%><%if request.cookies(eChsys)("key")="selfreg" and fabiaocheck="1" then%>1<%else%><%if request.cookies(eChsys)("key")="smallmaster" and rs5("usecheck")="1" then%>1<%else%>0<%end if%><%end if%><%end if%>"><%rs5.close%>
<input name="encode" type="hidden" value="html">
<input type="button" value=" 返 回 " onClick="javascript:history.go(-1)" class="unnamed5" >&nbsp;
<input type="submit" value=" 修 改 " name="Submit" class="unnamed5" OnClick="fnPreHandle()" >&nbsp;
