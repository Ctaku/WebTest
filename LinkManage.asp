...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="bigmaster" or request.cookies(eChsys)("ManageKEY")="check" or request.cookies(eChsys)("ManageKEY")="typemaster") then
	...
	if linkmana="1" or (request.cookies(eChsys)("ManagePurview")="99999" and request.cookies(eChsys)("purview")="99999") then
		...
		if linkpass="1" then
			rs.Source="select * from "& db_EC_Link_Table &" order by ID desc"
		else
			rs.Source="select * from "& db_EC_Link_Table &" where pass=0 order by ID desc"
		end if
...
<tr bgcolor=ffffff>
	...
	<td align=center  > 
		<%if rs("linktype")=2 then%>首页LOGO链接
			<%else%>
				<%if rs("linktype")=1 then%>其他LOGO链接
					<%else%>
						<%if rs("linktype")=0 then%>首页文字链接
							<%else%>其他文字链接
								<%end if%>
						<%end if%>
				<%end if%>
	...
	<td height="35" align=center valign="middle" > 
		<%if rs("linktype")=1 or rs("linktype")=2 then%><a target="_blank" title="点击查看原图" href="<%=rs("logo")%>"><img src="<%=rs("logo")%>" border=0 width=88 height=31></a><%else%>文字链接<%end if%>
	</td>
	<td align=center  >
	...
	<td align=center  >
		...
		<input type=submit value="<%if rs("pass")="0" then%>通过<%else%>取消<%end if%>审核" class=button onMouseOver="window.status='按这个按钮<%if rs("pass")="0" then%>通过<%else%>取消<%end if%>审核网站';return true;" onMouseOut="window.status='';return true;"target=_blank title="按这个按钮<%if rs("pass")="0" then%>通过<%else%>取消<%end if%>审核网站" >
	</td>
	<td align="center"  >
