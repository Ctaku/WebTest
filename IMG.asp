...
rs3.Source ="select top " & top_img & " * from "& db_EC_News_Table &" where picnews=1 and checkked=1 order by NewsID DESC"
end if
if session("key")="" then
rs3.Source ="select top " & top_img & " * from "& db_EC_News_Table &" where picnews=1 and checkked=1 order by NewsID DESC"
end if
...
%>
 <%if rs3("picname")<>"" then%>
    <td height=31 align="center"><a href="E_ReadNews.asp?newsid=<%=rs3("newsid")%>" target="_blank" title="<%=rs3("title")%>"><img src="<%=rs3("picname")%>" width="160" height="120" border="0"></a></td>
<%end if
rs3.MoveNext
wend
%>
