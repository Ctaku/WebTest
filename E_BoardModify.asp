...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="bigmaster" or request.cookies(eChsys)("ManageKEY")="check" or request.cookies(eChsys)("ManageKEY")="typemaster") then
	...
	sql1="select * from "& db_EC_Board_Table &" where id="&ChkRequest(request("id"),1)	'防注入
...
<form method="POST" action="E_BoardModSave.asp?id=<%=ChkRequest(request("id"),1)%>" OnSubmit="return checkdata()" onReset="return ResetForm();" name="form1">
<tr class="TDtop">
...
<tr> 
	...
	<td width="685" bgcolor="#FFFFFF">&nbsp;
		<input name="title" value="<%=rs1("title")%>" id=me type="TEXT" size=60  onMouseOver="window.status='在这里输入要添加的公告标题，必填';return true;" onMouseOut="window.status='';return true;" title='在这里输入要添加的公告标题，必填'  >
	</td>
</tr>
<tr>
	...
	<td width="685" bgcolor="#FFFFFF">&nbsp;&nbsp;
		...
		<script language="javascript">
			document.write ('<iframe src="E_BoardTextBox.asp?action=modify&id=<%=request("id")%>" id="message" width="650" height="300" align=center></iframe>')
...
<tr class="TDtop" align="center"> 
	<td colspan="2" height="25">
		<input name="upload" type="hidden" value="<%=request.cookies(eChsys)("username")%>">
		<input type="button" value=" 返 回 " onClick="javascript:history.go(-1)" class="unnamed5"    >&nbsp;&nbsp;
		<input type="submit" value=" 添 加 " name="Submit" class="unnamed5" OnClick="document.form1.Content.value=frames.message.document.body.innerHTML;"  >&nbsp;
		<input type="reset" value=" 重 置 " name="Reset" class="unnamed5"  >
