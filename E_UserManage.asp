...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="typemaster" or request.cookies(eChsys)("ManageKEY")="bigmaster") then
	...
	oskey=ChkRequest(request("oskey"),0)	'防注入
	...
	if request.cookies(eChsys)("key")="super" then
		...
		if oskey="" then
			rst.Source="select * from "& db_User_Table &" where adder='"&request.cookies(eChsys)("username")&"' order by "& db_User_ID &""
		else
			rst.Source="select * from "& db_User_Table &" where adder='"&request.cookies(eChsys)("username")&"' and oskey='"&oskey&"' order by "& db_User_ID &""
		end if
	end if
...
<table border="1" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFDFBF"  bgcolor="#FFFFFF" width="100%" id="AutoNumber1">
<tr align="center"  height=25>
	<td colspan="12" height=25 class="TDtop">┊ <B>用户管理</B> ┊ </td>
</tr>
<tr align="center"  height=25>
	<td colspan="12" height=25 >
		<p align=center>用户列表(<font color=red>红色</font>表示当前用户)，点击用户名可查看更多信息 <a href="useradd1.asp">添加用户</a>  <a href="E_UserManage.asp">全部用户</a>
	</td>
</tr>
<tr align="center" class="TDtop1"  height=20> 
  ...
  <%response.write "<form name='formdel' method='post' action='E_UserManage.asp'>"
  for i=1 to PageSize
		if not rst.eof then
			if rst("purview")<>"99999" or request.cookies(eChsys)("purview")="99999" then
				response.write "<tr align='center' height=20>"& chr(10)
				if (request.cookies(eChsys)("key")="bigmaster" and rst("oskey")="smallmaster") or (request.cookies(eChsys)("key")="typemaster" and (rst("oskey")="bigmaster" or rst("oskey")="smallmaster")) or request.cookies(eChsys)("key")="super" then
					...
					if rst(db_User_Name)=request.cookies(eChsys)("username") then
					...
					if rst("oskey")="super" and rst("purview")="99999" then
						...
						select case oskey1
							...
							case "check" response.write "<a href='E_UserManage.asp?oskey="& oskey1 &"'>审核用户</a>"
							case "typemaster" response.write "<a href='E_UserManage.asp?oskey="& oskey1 &"'>总栏用户</a>"
							case "bigmaster" response.write "<a href='E_UserManage.asp?oskey="& oskey1 &"'>大类用户</a>"
							case "smallmaster" response.write "<a href='E_UserManage.asp?oskey="& oskey1 &"'>小类用户</a>"
							case "selfreg" response.write "<a href='E_UserManage.asp?oskey="& oskey1 &"'>注册用户</a>"
						end select
					end if
					response.write "</td>"& chr(10)
					...
					response.write "<td align='center' bgcolor='#FFFFFF'>"
					if trim(request.cookies(eChsys)("username"))=trim(rst(db_User_Name)) then
						response.write "----"
					else
				...
				response.write "</td>"
					...
					if trim(request.cookies(eChsys)("username"))=trim(rst(db_User_Name)) or rst("oskey")<>"selfreg" then
						response.write "--"
					else
	...
	<input type=checkbox name=chkall value=on onClick="CheckAll(this.form)" style="font-size: 9pt;  color: #000000; background-color: #ffff; solid #FEEBE4">选中所有显示用户&nbsp;
	<input type=submit name=action1 onClick="{if(confirm('删除选定的用户吗？')){this.document.check.submit();return true;}return false;}" value="删除用户" >
...
<p align=center height=25>共 <%=total%> 名，当前第 <%=Mypage%>/<%=Maxpages%> 页，每页 <%=MyPageSize%> 名 
	<%
	if oskey="" then
		url="E_UserManage.asp?"
	else
		url="E_UserManage.asp?oskey="& oskey &"&"
	end if
	PageNextSize=int((MyPage-1)/PageShowSize)+1
	Pagetpage=int((total-1)/PageSize)+1
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bordercolor="#FFDFBF" bgcolor="#FFFFFF" id="AutoNumber1" style="border-collapse: collapse">
<tr>
<td>
<font color="#FF0000">
...
<form method="POST" action="E_UserManage.asp" name="SeahUser"  onSubmit="return CheckFormUserLogin();">
<tr align="center" height=20>
<td colspan=13>
<input type=hidden name="page" value=<%=MyPage%>>
用户名：<input name="UserName" <%if SeachUserName<>"" then%>value=<%=SeachUserName%><%end if%> size="15" font face="宋体" style="font-size: 9pt; background-color:#FEEBE4">
<input type="submit" name="Submit" value="查找"  title="查找用户">&nbsp;&nbsp;
	<%if SeachModel="" or SeachModel=1 then
		...
		rstsql="select * from "& db_User_Table &" where "& db_User_Name &" = '"& SeachUserName &"' order by "& db_User_ID
		Sea_Mod="精确查找："
	else
		...
		rstsql="select * from "& db_User_Table &" where "& db_User_Name &" like '"& SeachUserName &"%' Or " & db_User_Name &" like '%"& SeachUserName & "'"
		Sea_Mod="模糊查找："
	end if
    %>
</td>
	...
	if SeachUserName<>"" then
		...
		if not rst1.eof then
			%>
			<form method="POST" action="E_UserManage.asp" name="SeahUsertest" >
		...
		<%do while not rst1.eof
			if rst1("purview")<>"99999" or request.cookies(eChsys)("purview")="99999" then
				response.write "<tr align='center' height=20>"& chr(10)
				if (request.cookies(eChsys)("key")="bigmaster" and rst1("oskey")="smallmaster") or (request.cookies(eChsys)("key")="typemaster" and (rst1("oskey")="bigmaster" or rst1("oskey")="smallmaster")) or request.cookies(eChsys)("key")="super" then
					...
					if rst1(db_User_Name)=request.cookies(eChsys)("username") then
					...
					response.write "<td align='center' bgcolor='#FFFFFF'>"
					if trim(request.cookies(eChsys)("username"))=trim(rst1(db_User_Name)) then
						response.write "----"
					else
				...
				response.write "</td>"
					response.write "<td align='center'>"
					if trim(request.cookies(eChsys)("username"))=trim(rst1(db_User_Name)) or rst1("oskey")<>"selfreg" then
						response.write "--"
					else
