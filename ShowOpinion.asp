...
<title><%=copyright%><%=version%>&nbsp;<%=ver%> - 局长信箱</title>
<style type="text/css">
<!--
.STYLE4 {color: #FF0000}
-->
...
<div align="center">
<form action=E_ReplyOpinionSave.asp method=post name=form1 OnSubmit="return checkdata()" onReset="return ResetForm();">	
  <table width="100%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolor="<%=border%>">
    <tr class="TDtop">
      <td height="27" colspan="2" class="TDtop"><div align="center">┊ <B>查看发言内容</B> ┊</div></td>
    </tr>
    <tr>
      <td width="25%" height="22"><div align="right">发言日期：</div></td>
    ...
    <tr>
      <td height="24"><div align="right"><strong><span class="STYLE4">*</span>是否公开发言：</strong></div></td>
      <td><%if rs("ClassID")=0 then%>
      <font color=red><strong>不希望公开发言，请回复到我的信箱！</strong></font>
      <%else%><font color=red>可以公开</font><%end if%></td>
    </tr>
	<tr>
      <td height="24"><div align="right"><strong><span class="STYLE4">*</span>发言版块：</strong></div></td>
      <td><%if rs("OpinionClass")=1 then%><font color=red><strong>各抒己见</strong></font><%end if%>
	  <%if rs("OpinionClass")=2 then%><font color=red><strong>发展建议</strong></font><%end if%>
	  <%if rs("OpinionClass")=3 then%><font color=red><strong>百姓生活</strong></font><%end if%>
	  <%if rs("OpinionClass")=4 then%><font color=red><strong>重大建设建议</strong></font><%end if%>
	  </td>
    </tr>
    <tr>
      <td height="24"><div align="right"><strong><span class="STYLE4">*</span>发言主题：</strong></div></td>
      <td><%=trim(rs("Topic"))%></td>
    </tr>
    <tr>
      <td height="45"><div align="right"><strong><span class="STYLE4">*</span>发言内容：</strong></div></td>
      <td><%=trim(rs("content"))%></td>
    </tr>
    <tr>
      <td height="31"><div align="right" class="STYLE4"><strong>回复时间：</strong></div></td>
      <td>
    ...
    <tr>
      <td height="34"><div align="right" class="STYLE4"><strong>回复内容：</strong></div></td>
      <td>
	  				<%if trim(rs("ReplyContent"))<>"" then %>
    ...
    <tr>
      <td height="96"><div align="right">回复内容：</div></td>
      <td height="96">
	  <br>
...
<iframe style="top:2px" ID="UploadFiles" src="Uploadfile1.htm" frameborder=0 scrolling=no width="420" height="40"></iframe>
                            <script language="javascript">
		document.write ('<iframe src="E_ReplyOpinionBox.asp?action=new" id="message" width="420" height="200" align=left></iframe>')
    ...
    <tr>
      ...
      <input type="hidden" name="OpinionID" value="<%=cint(request("OpinionID"))%>">	
      <input type="submit" value=" 回 复" name="Submit" onClick="document.form1.ReplyContent.value = frames.message.document.body.innerHTML;">
     &nbsp;&nbsp;
