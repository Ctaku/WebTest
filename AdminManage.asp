...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="typemaster" or request.cookies(eChsys)("ManageKEY")="bigmaster")  THEN
	...
	if (request.form("action")="del") and (request.form("selectdel")<>"") then
		if (request.cookies(eChsys)("ManageKEY")="super") then
			conn.execute "delete from "& db_EC_Manage_Table &" where "& db_ManageUser_ID &" in ("&request.form("selectdel")&")"
	...
	if request.cookies(eChsys)("ManageKEY")="super" then
		if oskey="" then
			rst.Source="select * from "& db_EC_Manage_Table &" order by "& db_ManageUser_ID &""
		else
			rst.Source="select * from "& db_EC_Manage_Table &" where oskey='"&oskey&"' order by "& db_ManageUser_ID &""
		end if
	else
		if oskey="" then
			rst.Source="select * from "& db_EC_Manage_Table &" where "& db_ManageUser_Name &"='"&request.cookies(eChsys)("username")&"' order by "& db_ManageUser_ID &""
		else
			rst.Source="select * from "& db_EC_Manage_Table &" where "& db_ManageUser_Name &"='"&request.cookies(eChsys)("username")&"' and oskey='"&oskey&"' order by "& db_ManageUser_ID &""
		end if
	end if
...
<table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFDFBF" bgcolor="#FFFFFF" id="AutoNumber1" style="border-collapse: collapse">
   <tr align="center"  height=25>
		  <td colspan="12" height=25 ><p align=center>管理用户列表(<font color=red>红色</font>表示当前管理用户)，点击管理用户名可查看更多信息 <a href="AdminAdd1.asp">添加管理用户</a>  <a href="AdminManage.asp">全部管理用户</a><br></td>
  </tr>
  <tr align="center"  height=25>
  ...
  <tr align="center" class="TDtop1"  height=20> 
	...
	<td width="4%" >选择</td>
		...
		<%response.write "<form name='formdel' method='post' action='AdminManage.asp'>"
			for i=1 to PageSize
				if not rst.EOF then
					if rst("Purview")<>"99999" or request.cookies(eChsys)("ManagePurview")="99999" then
						response.write "<tr align='center' height=20>"& chr(10)
							...
							if rst(db_ManageUser_Name)=request.cookies(eChsys)("username") then
	...
	<input type=checkbox name=chkall value=on onClick="CheckAll(this.form)" style="font-size: 9pt;  color: #000000; background-color: #ffff; solid #FEEBE4">选中所有显示管理用户&nbsp;
	<input type=submit name=action1 onClick="{if(confirm('删除选定的管理用户吗？')){this.document.check.submit();return true;}return false;}" value="删除管理用户" >
...
管理用户名：<input name="UserName" <%if SeachUserName<>"" then%>value=<%=SeachUserName%><%end if%> size="15" font face="宋体" style="font-size: 9pt; background-color:#FEEBE4">
<input type="submit" name="Submit" value="查找"  title="查找管理用户">&nbsp;&nbsp;
			<%if SeachModel="" or SeachModel=1 then
				...
				rstsql="select * from "& db_EC_Manage_Table &" where "& db_ManageUser_Name &" like '"& SeachUserName &"' order by "& db_ManageUser_ID
				Sea_Mod="精确查找："
			else
			...
			if SeachUserName<>"" then
				...
				if not rst1.eof then
					...
					if rst1("purview")<>"99999" or request.cookies(eChsys)("ManagePurview")="99999" then
						response.write "<tr align='center' height=20>"& chr(10)
						...
						if rst1(db_ManageUser_Name)=request.cookies(eChsys)("username") then
						...
						response.write "</td>"& chr(10)
						response.write "<td align='center' bgcolor='#FFFFFF'>"
						if trim(request.cookies(eChsys)("username"))=trim(rst1(db_ManageUser_Name)) then
							response.write "----"
						else
