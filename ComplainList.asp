...
<tr class="TDtop">
	<td colspan="9" height=25 class="TDtop" align="center">┊ <B>投诉举报管理</B> ┊ </td>
</tr>
<tr>
<form method="post" name="myform" action="Admin_Complain_Result.asp">
	<td colspan=9  align=center height=25>
      ...
      <input type="submit" name="Submit" value="搜 索" class="login_username" onMouseOver ="this.style.backgroundColor='#F3EAD5'" onMouseOut ="this.style.backgroundColor='#FFFFFF'" >	</td>
</form>
</tr>
<tr>
	<td colspan=9  align=center height=25>共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 页，每页 <%=MyPageSize%> 条</td>
</tr>
...
<tr bgcolor=ffffff>
	...
	<td align=center><%if rs("Display")=0 then%><font color=red>不公开</font><%else%><font color=red>公开</font><%end if%></td>
	...
	<td align=center><%if trim(rs("ReplyContent"))<>"" then %>
	...
	<td align=center >
		...
		<a href=DelComplain.asp?ComplainID=<%=rs("ComplainID")%> style="font-size: 9pt;  color: #000000; background-color: #ffffff; solid #FEEBE4" onMouseOver ="this.style.backgroundColor='#FEEBE4'" onMouseOut ="this.style.backgroundColor='#ffffff'">删除</a>	</td>
</tr>
				<%
				rs.MoveNext
			end if
		...
		%>
<tr>
	<td colspan=9  align=center height=25>共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 页，每页 <%=MyPageSize%> 条 
		<%
