...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="typemaster" or request.cookies(eChsys)("ManageKEY")="bigmaster") then
	...
	OpinionID=ChkRequest(Request.Form("OpinionID"),1)	'防注入
	button_value=trim(Request.Form("alert_button"))
	if button_value="确定" then
		conn.execute("delete from "& db_EC_Opinion_Table &" where OpinionID=" & OpinionID)
	else
		Response.Redirect "E_OpinionList.asp"
	end if

	response.write "<meta http-equiv=""refresh"" content="""&freetime&";url=E_OpinionList.asp"">"
	Show_Message("发言 ID =["& OpinionID &"]删除成功！!<br><br>"&freetime&"秒钟后返回上页!</font>")
	response.end
	else
