...
<%
if request.cookies(eChsys)("ManageKEY")<>"super" or request.cookies(eChsys)("ManagePurview")<>"99999" then
	Show_Err("对不起，您的后台管理权限不够！")
	response.end
else
	if system="1" or request.cookies(eChsys)("ManagePurview")="99999" then
	...
	Response.Buffer=False
		...
		if Request.form("Check_Save")="ok" then
			response.cookies(eChsys)("C_Attach")=Request.form("Check_Attach")
			response.cookies(eChsys)("C_News")=Request.form("Check_News")
			response.cookies(eChsys)("C_UserFace")=Request.form("Check_UserFace")
		end if

		Check_Attach=Request.cookies(eChsys)("C_Attach")
		Check_News=Request.cookies(eChsys)("C_News")
		Check_UserFace=Request.cookies(eChsys)("C_UserFace")
...
<div align="center">
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFDFBF" width="100%" id="AutoNumber1">
<form action='Admin_UpFileManage.asp' method=post name=check>
<tr>
	<td height="25" colspan="5" align="center" valign="middle"  class="TDtop">┊ 所 有 文 件 列 表 / 慎 重 操 作 /┊ </td>
</tr>
</table>
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFDFBF" width="100%" id="AutoNumber1">
<tr>
	<td width='30%' height=20 align='center' class="TDtop1">文件名</td>
	<td width='30%' align='center' class="TDtop1">引用验证</td>
	<td width='15%' align='center' class="TDtop1">文件大小</td>
	<td width='15%' align='center' class="TDtop1">文件属性</td>
	<td width='10%' align='center' class="TDtop1">功能</td>
</tr>
</table>
		<%
		dim theFolder
			...
			for each x in theFiles
				if i>(MyPageSize*(Mypage-1)) and i<(MyPageSize*Mypage+1) then
					...
					if Check_Attach="1" then
						...
						sql ="select * from "& db_EC_Attach_Table &" where filename like '%"& x.name &"%'" 
						rs.open sql,conn,1,1
						do until rs.eof
							'在上传附件表中搜索是否有上传记录
							contain=contain&"<a href='E_ReadNews.asp?NewsID="& rs("newsid") &"' target='_blank' title='在上传附件表中显示该附件在ID号为"& rs("newsid") &"的文章中被引用'>上传记录：" & rs("newsid") & "号文章</a><br>"
							rs.MoveNext
						loop
						rs.close
						set rs=nothing
					...
					if Check_News="1" then					
						Set rs = Server.CreateObject("ADODB.Recordset")
						sql ="select * from "& db_EC_News_Table &" where Content like '%"& x.name &"%'"
						rs.open sql,conn,1,1
						if not rs.eof then
							contain= contain & x.name &"符合内容记录["& rs.RecordCount &"]<a href='E_ReadNews.asp?NewsID="& rs("newsid") &"' target='_blank' title='该附件在ID号为"& rs("newsid") &"的文章内容中被引用'><font color='green'>新闻内容：" & rs("newsid") & "号文章</font></a><br>"
							do until rs.eof
								'在新闻的具体内容中搜索是否被引用
								cont_search = Instr(1, rs("Content"),x.name, 1)
								if cont_search>0 then
									contain=contain&"<a href='E_ReadNews.asp?NewsID="& rs("newsid") &"' target='_blank' title='该附件在ID号为"& rs("newsid") &"的文章内容中被引用'><font color='green'>新闻内容：" & rs("newsid") & "号文章第"& cont_search &"字处</font></a><br>"
								end if
								rs.MoveNext
							loop
						end if
...
<tr>
	<td height="20" colspan="5" align=right class="TDtop1">
		...
		<input type=submit name=action onClick="{if(confirm('删除选定的上传文件吗？')){this.document.check.submit();return true;}return false;}" value="删除文件" >
	</td>
</tr>
</form>
<tr>
...
<tr>
	<td height="25" colspan="5" bgcolor=#ffffff align=center>
		...
		<input type=submit name=Check_Sel value="设定" >
	</td>
</tr>
</form>
<tr>
