...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="typemaster" or request.cookies(eChsys)("ManageKEY")="bigmaster") then
	...
	if plmanage="1" or (request.cookies(eChsys)("ManagePurview")="99999" and request.cookies(eChsys)("purview")="99999") then
	...
	if reviewcheckshow="1" then
		rs.Source="select * from "& db_EC_Review_Table &" where newsid is not null and newsid > 0 order by reviewID desc"
	else
		rs.Source="select * from "& db_EC_Review_Table &" where passed=0 and newsid is not null newsid > 0 order by reviewID desc"
	end if
...
<tr bgcolor=ffffff>
	...
	<td align=center><a href='E_ReadNews.asp?NewsID=<%=rs("NewsID")%>' target="_blank"><%=rs("NewsID")%></a></td>
	...
	<td align=center >
		...
		<input type=submit value="<%if rs("passed")=0 then%>通过<%else%>取消<%end if%>审核" class=button target=_blank title="按这个按钮<%if rs("passed")=0 then%>通过<%else%>取消<%end if%>审核" >
		<a href=DelReView2_alert.asp?ReViewID=<%=rs("ReViewID")%> style="font-size: 9pt;  color: #000000; background-color: #ffffff; solid #FEEBE4" onMouseOver ="this.style.backgroundColor='#FEEBE4'" onMouseOut ="this.style.backgroundColor='#ffffff'">删除</a>
	</td>
</tr>
</form>
				<%
