...
if reviewcheckshow="1" then
	rs.Source="select * from "& db_EC_Review_Table &" order by reviewID desc"
else
	rs.Source="select * from "& db_EC_Review_Table &" where passed=0 order by reviewID desc"
end if
...
<td width=7% align=center><a href="E_ReadNews.asp?newsid=<%=rs("newsid")%>" target="_blank"><%=rs("newsid")%></a></td>
