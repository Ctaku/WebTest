...
mtDropDown.backgroundColor = "#0D82BF"; 
...
<tr> 
	<td class="top">
<%sqltype="select * from "& db_EC_Type_Table & " where E_dh_typeview=1 order by E_typeorder"			'选择分类
...
if rstype.bof and rstype.eof then 
	...
	rstype.movefirst
		do while not rstype.eof
			...
			'strtype = strtype & "<span style=' height:14px;color=#FFFFFF;FONT-WEIGHT: bold;FONT-SIZE:14px'><a class=MenuType href='"& rstype("url") &"'>"&rstype("E_typename")&"</a>&nbsp;|</span>"& chr(10) &""
			strtype = strtype & "<span style=' height:14px;color=#FFFFFF;FONT-WEIGHT: bold;FONT-SIZE:14px'><a class=MenuType href='"& rstype("url") &"' id=menu"& Type_i &" title='"& rstype("typecontent") &"'>"& rstype("E_typename") &"</a>&nbsp;|</span>"& chr(10) &""
			else
			strtype = strtype & "<span style=' height:14px;color=#FFFFFF;FONT-WEIGHT: bold;FONT-SIZE:14px'><a class=MenuType href='E_Type.asp?E_typeid="& rstype("E_typeid") &"' id=menu"& Type_i &" title='"& rstype("typecontent") &"'>"& rstype("E_typename") &"</a>&nbsp;|</span>"& chr(10) &""
			end if
			rstype.movenext
			Type_i=Type_i+1
		loop
		...
		do while not rstype.eof
			...
			if rsBigClass.bof and rsBigClass.eof then
				...
				do while not rsBigClass.eof
					...
					response.write "	menu"& Type_i &".addItem('- "& rsBigClass("E_BigClassName") &"','E_BigClass.asp?E_typeid="& rsBigClass("E_typeid") &"&E_BigClassID="& rsBigClass("E_BigClassID") &"'); "& chr(10) &""
					...
					if B_BG=3 then	'判断是否3
						...
						if rsSmallClass.bof and rsSmallClass.eof then
							...
							do while not rsSmallClass.eof
								...
								SubMenuContent = SubMenuContent + "	subMenu"& second_i &".addItem('- "& rsSmallClass("E_smallclassname") &"','E_SmallClass.asp?E_typeid="& rstype("E_typeid") &"&E_BigClassID="& rsSmallClass("E_BigClassID") &"&E_SmallClassID="& rsSmallClass("E_SmallClassID") &"');"& chr(10) &""
								end if
