...
if not(request.cookies(eChsys)("ManageKEY")="super" or request.cookies(eChsys)("ManageKEY")="typemaster" or request.cookies(eChsys)("ManageKEY")="bigmaster") then
	...
	if reviewcheckshow="1" then
		rs.Source="select * from "& db_EC_Review_Table &" where newsid is not null and newsid < 1 order by reviewID desc"
	else
...
<tr bgcolor=ffffff>
	...
	<td align="left"><a href='E_GuestBook.asp?ReViewID=<%=rs("ReViewID")%>' target=_blank><%=htmlencode(DisplayContent)%>...</a></td>
	...
	<td align=center >
		...
		<a href=DelReView4_GuestBook.asp?ReViewID=<%=rs("ReViewID")%> style="font-size: 9pt;  color: #000000; background-color: #ffffff; solid #FEEBE4" onMouseOver ="this.style.backgroundColor='#FEEBE4'" onMouseOut ="this.style.backgroundColor='#ffffff'">删除</a>
	</td>
</tr>
</form>
				<%
