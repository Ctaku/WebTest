...
%>
              ...
              <%end if%>
              　<a href=javascript:opw('option.asp?id=<%=adsrs("id")%>&job=delit','del<%=adsrs("id")%>',300,140)>删除</a>
			    <br><br><a href=javascript:opw('ipdel.asp?id=<%=adsrs("id")%>&job=delit','del<%=adsrs("id")%>',300,140)>IP删除</a>
			  </td>
      </tr>
      <tr>
